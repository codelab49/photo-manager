...
export async function POST(request: NextRequest) {
  try {
    ...
    const watermarkedBuffer = await addWatermark(buffer, {
      text: "PREVIEW ONLY",
      opacity: 0.3,
      fontSize: Math.min(metadata.width || 800, metadata.height || 600) / 15
    });

    // Upload watermarked preview
    const previewResult = await uploadToGCS(
      watermarkedBuffer,
      `preview_${file.name}`,
      "image/jpeg",
      `previews/${sessionId}`
    ...
    // Create thumbnail from original (no watermark on small thumbnails)
    ...
    const photo = await prisma.photo.create({
      data: {
        ...
        previewPath: previewResult.success ? previewResult.publicUrl : null,
